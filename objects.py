	...
	for i in range(len(precedences)):
		if i+1 in ended_tasks:
			...
			if precedences[i]==[]:
				...
				ind=1
				for prec in precedences[i]:
					if prec in ended_tasks:
						continue
					else:
						ind=0
						break
				if ind==1: candidates.append(i+1)
	return candidates
	
def read_file(filename):
	def precedences(tuplist, N):
		llista_prec=[]
		for i in range(N):
			llista_prec.append([])
		for tup in tuplist:
	...
	def successors_time(llistaSucc, durations, N):
		succAcum=[]
		def auxTR(ind):
			succInd=[]
			if llistaSucc[ind]==[]:
	...
	data['precedences']=precedences(preced,data['N'])
	data['successors']=successors(preced,data['N'])
	...
	for i in range(data['T']):
		tascaTreball=[int(x) for x in f.readline().split('*')]
		workers_by_task.append(tascaTreball)
	tasks_by_worker=[]
	for i in range(data['O']):
		treballador=[]
		for l in range(data['T']):
			if i+1 in workers_by_task[l][1:]: treballador.append(l+1)
		tasks_by_worker.append(treballador)
	...
	data['successors_time'], data['numSucc']=successors_time(data['successors'], data['durations'], data['N'])
	f.close()
	return data	
...
class AssemblyLine(object):
	...
	def check_time(self, task, ws):
			if self.data['durations'][task-1]+ws.temps <= self.data['TCM']: 
				return True
			else: 
				return False
	...
	def check_precedences(self, task, ws):
		...
		if task in task_candidates(self.data['precedences'], ended_tasks):
			return True
		else: 
			return False

	def check_successors(self, task, ws_tgt_idx, ws_idx = 0):
	...
	def add_task(self, task, ws):
		...
		for treb in range(self.data['O']):
			if compare_lists (ws.tipusTasques, self.data['tasks_by_worker'][treb]):
	...
	def substitution_workers(self, end = 0):
		...
		def check_worker(tipusTasques):
			...
			empleat=0
			for treb in range(self.data['O']):
				if compare_lists (tipusTasques, self.data['tasks_by_worker'][treb]):
					if self.data['workers_cost'][treb+1] < valor:
						valor=self.data['workers_cost'][treb+1]
						empleat=treb+1
			if empleat != 0:
	...
	def move_task_backward(self,task, ws, ws_tgt):
		...
		valor=max(self.data['workers_cost'])
		for treb in range(self.data['O']):
			if compare_lists (ws_tgt.tipusTasques, self.data['tasks_by_worker'][treb]):
				if self.data['workers_cost'][treb+1]<valor:
					valor=self.data['workers_cost'][treb+1]
