...
def prog(fileR, fileW, iW=0, tmax=240.0):
	print('\n')
	print(fileR)
	print('----------------------------------')
	if iW==1: sol=open(fileW,'w')
	init=time.perf_counter()
	data=read_file(fileR)
	r1=algoritme1(data)
	rf=r1
	if iW==1: 
		sol.write("*".join([str(rf.cost_AL()), str(time.perf_counter()-init)])+"\n")
		print("*".join([str(rf.cost_AL()), str(time.perf_counter()-init)]))
	...
	while time.perf_counter()-init < tmax*2.0/3.0:
	...
	while time.perf_counter()-init < tmax-5:
	...
	if iW==1:
		sol.write("*".join([str(rf.cost_AL()), str(time.perf_counter()-init)])+"\n")
		print("Solucio final: "+("*".join([str(rf.cost_AL()), str(time.perf_counter()-init)])+"\n"))
		writeFile(rf, sol)
	return rf

def writeFile(cadena, openedFile):
	f=openedFile
	TC=0
...
def main():
	...
	if quest == 0:
		...
		eje = input('Nombre identificador de los ejemplares (p.ej: ejemplar_prueba_): ')
		sol = input('Nombre identificador de las soluciones (p.ej: ejemplar_solucion_): ')
